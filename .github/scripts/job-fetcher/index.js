...
/**
 * Main entry point for the job fetcher system - New-Grad-Data-Science-Jobs-2026
 *
 * Fetches data science jobs from centralized aggregator and generates README.
 * Replaced JSearch direct fetch with aggregator consumer (2026-02-18).
 *
 * Pipeline:
 * 1. Fetch data science jobs from aggregator (jobs-data-2026/all_jobs.json)
...
async function main() {
  try {
    console.log('🚀 Starting Data Science jobs fetching system...');
    ...
    if (jobs.length === 0) {
      console.log('⚠️  No data science jobs fetched from aggregator');
      console.log('   Check that all_jobs.json exists in jobs-data-2026');
    }

    console.log(`\n✅ Fetched ${jobs.length} data science jobs from aggregator`);
